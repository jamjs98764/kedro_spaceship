...
def lint(files):
    ...
    if not files:
        files = ("src/tests", "src/spaceship_kedro_package")
...
def build_docs(open_docs):
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=spaceship_kedro_package"]
    )
...
def _build_jupyter_command(
    ...
    if not all_kernels:
        project_name = "spaceship_tutorial"
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    kedro_package_name = "spaceship_kedro_package"
